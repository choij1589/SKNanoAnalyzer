...
if [[ $RUN == "Run2" ]]; then
    ...
    SKNano.py -a CRPromptSelector -i SampleLists/Run2NanoV9/TriLepton.txt -n 10 -r ${RUN} --userflags ${CHANNEL},RunSyst --python --memory 5000
    SKNano.py -a CRPromptSelector -i Skim_TriLep_WZTo3LNu_amcatnlo -n 10 -r ${RUN} --userflags ${CHANNEL},RunNoWZSF,RunSyst --python --memory 5000
elif [[ $RUN == "Run3" ]]; then
    if [[ $CHANNEL == "Run1E2Mu" ]]; then
        DATASTREAM="Skim_TriLep_MuonEG"
    elif [[ $CHANNEL == "Run3Mu" ]]; then
